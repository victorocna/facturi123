...
function email_parola(){
	if (!$('#ascuns').val()){
		...
		$.ajax({type:'GET',url:'/includes/functii123.php?op=email_parola'+query,success:function(raspuns){
				if (raspuns == 1){
					notify_bars(20,'Un email cu noua parola asociata utilizatorului tau a fost trimis la adresa <span class="activ">'+$('#email').val()+'</span>');
					$('#email').val(''); $('#user').val('');
				}
...
<form id="form" onsubmit="return false;">
	...
	<div class="span-16 box-margin-m">
		...
		<div class="span-11">
			<input class="after span-11 validator-required validator-email" type="text" id="email" tips="<div id='toggle-tips' class='span-7 tips-e'><strong>Informatie necesara</strong><br>Dupa introducerea adresei de email, iti vom trimite o parola noua ce poate fi modificata ulterior din sectiunea <span class='activ-b'>Contul tau</span></div>" autocomplete="off">
		</div>
	</div>
	<div class="span-16 box-margin-m">
		<div class="before span-2">Utilizator</div>
		<div class="span-11">
			<input class="after span-11 validator-required" type="text" id="user" tips="<div id='toggle-tips' class='span-7 tips-e'><strong>Informatie necesara</strong><br>Completeaza utilizatorul asociat parolei tale</div>" autocomplete="off">
		</div>
	</div>
	<div class="span-16 box-margin-m">
