...
function ValidatorRule (name, errorMessage, validationMethod){
	...
	this.errorMessage = errorMessage ? errorMessage : 'Date incorecte.';
	this.name = name;
};
...
jQuery.validationAide = {
	getDefaultValidationRules : function(){
		...
		rules.add('validator-required', 'Informatie necesara', function(fieldValue, fieldObj) {
			if (fieldValue == "")	return false;
			return true;
		});
		
		rules.add('validator-email', 'Adresa de email incorecta', function(fieldValue, fieldObj) {
			if (fieldValue == "")	return true;
			return /\w{1,}[@][\w\-]{1,}([.]([\w\-]{1,})){1,3}$/.test(fieldValue);
		});
		
		rules.add('validator-number', 'Numar incorect', function(fieldValue, fieldObj) {
			if (fieldValue == "")	return true;
			return (!isNaN(fieldValue) && !/^\s+$/.test(fieldValue));
		});
		
		rules.add('validator-digits', 'Doar cifre permise', function(fieldValue, fieldObj) {
			if (fieldValue == "")	return true;
			return !/[^\d]/.test(fieldValue);
		});
		
		rules.add('validator-alphanum', 'Doar litere (a-z) sau numere (0-9) permise', function(fieldValue, fieldObj) {
			if (fieldValue == "")	return true;
			return !/\W/.test(fieldValue);
		});
		
		rules.add('validator-date-ddmmyyyy', 'Data incorecta. Data trebuie sa fie de tipul dd/mm/yyyy', function(fieldValue, fieldObj) {
			if (fieldValue == "")	return true;
		...
		rules.add('validator-date-mmddyyyy', 'Data incorecta. Data trebuie sa fie de tipul  mm/dd/yyyy', function(fieldValue, fieldObj) {
			if (fieldValue == "")	return true;
			var regex = /^(\d{2})\/(\d{2})\/(\d{4})$/;
			if(!regex.test(fieldValue)) return false;
			var d = new Date(fieldValue.replace(regex, '$2/$1/$3'));
		...
		rules.add('validator-url', 'Adresa URL incorecta.', function(fieldValue, fieldObj) {
			if (fieldValue == "")	return true;
	...
	extendOptions : function(options){
		...
		if (typeof retval.summaryMessage == 'undefined')
			retval.summaryMessage = "Eroare la introducerea datelor! Va rog corectati urmatoarele:";
	  ...
	  if (fullOptions.showSummary){
					...
					for (var i=0; i<cssClasses.length; i++){
						...
						if (validationRule){
							...
							if (!retval){
								...
								var title = jQueryElm.attr("error");
		...
		if (!validatedOK && fullOptions.showSummary){
			...
			for(var i=0;i<messagesForSummary.length; i++){
				summaryHtml += "<li><a href=\"#\" onclick=\"jQuery('#" + fieldsForSummary[i] + "')[0].focus(); return false;\">" + messagesForSummary[i] + "</a></li>";
			}
			summaryHtml += "</ul>";
		...
		if (typeof postFieldValidation == 'function')
								postFieldValidation(validatedOK);
		//alert (postFieldValidation);
		return validatedOK;
		//alert (validatedOK);
	}
};
