...
function verifica_denumire(){
	if ($("#denumire").val()){
		var query = '&denumire='+$.trim($("#denumire").val())+'&tip_firma=9';
...
function save_firma(){
	...
	$.ajax({type:'GET',url:'/includes/functii.php?op=save_firma'+query,success:function(raspuns){
		if (raspuns){
			...
			window.top.notify_bars(10,'Platitorul <span class="uppercase activ">'+$('#denumire').val()+'</span> a fost salvat');
...
<div class="span-17" style="margin-bottom: 10px;">
	...
	<div class="span-3" style="text-align: right;"><button class="fg-button orange ui-corner-all" type="submit"><span class="button-text">Salveaza</span></button></div>
