...
function verifica_denumire(){
	if ($("#denumire").val()){
		var query = '&denumire='+$.trim($('#denumire').val())+'&tip_firma=9&id_firma='+<?php echo $_GET['id_firma']; ?>;
		$.ajax({type:'GET',dataType:'json',url:'/includes/functii.php?op=verifica_denumire'+query,success:function(raspuns){
				...
				if (raspuns.fault == '1'){
					$('#verifica-denumire').html('Platitorul exista deja');
					$('#denumire').attr('fault', '1').removeClass('img-yes').addClass('img-no');
					$('#toggle-tips').html('<div style="margin-bottom: 5px;"><strong>Atentie!</strong><br>Platitorul <span class="uppercase">'+$('#denumire').val()+'</span> a fost adaugat pe data de <strong>'+raspuns.data_add+'</strong></div><span>Vreti sa modificati datele platitorului? <a href="#" onmouseover="modify('+raspuns.id+')" id="modify" style="color: #00f !important;">Modifica</a></span>');
					$('input').blur();
					$('#denumire').focus();
				}
			}
...
function save_firma(){
	...
	$.ajax({type:'GET',url:'/includes/functii.php?op=update_firma'+query+'&platitor=1',success:function(raspuns){
		if (raspuns){
			window.top.$('body').find('#dialog_modify').dialog('close');
			window.top.notify_bars(10,'Datele platitorului <span class="uppercase activ">'+$('#denumire').val()+'</span> au fost modificate');
...
<div class="span-14 last"><div class="info-login ui-state-hover">Informatii firma</div></div>
...
<div class="span-14 last"><div class="info-login ui-state-hover">Informatii financiare</div></div>
<div class="span-20 form-row">
...
<div class="span-17" style="margin-bottom: 10px;">
	<div class="span-11" style="text-align: right;"><a href="javascript: confirm()" class="back">Inapoi</a></div>
