...
function verifica_denumire(){
	if ($("#denumire").val()){
		var query = '&denumire='+$.trim($("#denumire").val())+'&tip_firma=1&id_firma='+<?php echo $_GET['id_firma']; ?>;
...
function save_firma(){
	...
	$.ajax({type:'GET',url:'/includes/functii.php?op=update_firma'+query+'&client=1',success:function(raspuns){
		if (raspuns){
			window.top.$('body').find('#dialog_modify').dialog('close');
			window.top.notify_bars(10,'Datele clientului <span class="uppercase activ">'+$('#denumire').val()+'</span> au fost modificate');
