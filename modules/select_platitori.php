...
$i = 0;
if (isset($_GET['vl'])){
	$sql = $db->query('select * from firme where id_user="'.$_SESSION['id_user'].'" and tip_firma="9" and denumire like "'.$_GET['vl'].'%" order by denumire asc');
	if (mysql_num_rows($sql) > 0){
		...
		$sql2 = $db->query('select * from firme having locate(" '.$_GET['vl'].'",denumire) > 0 and id_user="'.$_SESSION['id_user'].'" and tip_firma="9" order by denumire asc');
		echo '<ul>';
	...
	$sql = $db->query('select * from firme where id_user="'.$_SESSION['id_user'].'" and tip_firma="9" order by denumire asc limit 20');
	if (mysql_num_rows($sql) > 0){
		echo '<ul>';
		while ($row = mysql_fetch_array($sql)){
			if ($i%2 == 0) echo '<div class="span-6 even">';
