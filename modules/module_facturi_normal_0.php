...
<div class="span-22 last" style="margin-left: 10px;">
	<table border="1" style="border-collapse: collapse; margin-bottom: 30px; border: solid 1px #b3b3b3;" id="tbl">
		<tr style="text-align: center; font-size: 1.2em;" class="ui-widget-header">
			...
			<td class="rows-l span-9">Denumire produs</td>
			<td class="rows-l span-25">UM</td>
			<td class="rows-l span-25">Cantitate</td>
			<td class="rows-l span-25" id="pret-h">Pret unitar</td>
			<td class="rows-l span-3">Valoare</td>
		</tr>
		<tr class="grey" style="text-align: center; font-size: 1em; padding: 0;">	
			<td class="rows-s span-1"><span class="rows-ie">0</span></td>
			<td class="rows-s span-9">1</td>
			<td class="rows-s span-25">2</td>
			<td class="rows-s span-25">3</td>
			<td class="rows-s span-25">4</td>
			<td class="rows-s span-3">5(3x4)</td>
		</tr>
<?php
if (isset($factura['facturi']['linii']['linie_attr'])){
	...
	echo '
		<tr class="tr-last tr-even" style="font-size: 1.2em;">
			...
			<td class="span-9 rnd">'.$rp['denumire'].'</td>
			<td class="span-25 rnd">'.$rp['unitate'].'</td>
			<td class="span-25 rnd-r">'.$factura['facturi']['linii']['linie']['cantitate'].'</td>
			<td class="span-25 rnd-r">'.$factura['facturi']['linii']['linie']['pret'].'</td>
			<td class="span-3 rnd-r">'.$factura['facturi']['linii']['linie']['valoare'].'</td>
		</tr>
	';
}
else{
	for ($i=0; $i<(count($factura['facturi']['linii']['linie'])-2); $i++){
		...
		echo '
			...
			<td class="span-9 rnd">'.$rp['denumire'].'</td>
			<td class="span-3 rnd">'.$rp['unitate'].'</td>
			<td class="span-25 rnd-r">'.$factura['facturi']['linii']['linie'][$i]['cantitate'].'</td>
			<td class="span-25 rnd-r">'.$factura['facturi']['linii']['linie'][$i]['pret'].'</td>
			<td class="span-3 rnd-r">'.$factura['facturi']['linii']['linie'][$i]['valoare'].'</td>
		</tr>
		';
	}
...
?>
			...
			<td colspan="2" class="span-5" style="font-weight: bold; text-align: right; padding: 13px 10px 13px 0;">Subtotal</td>
<?php
	echo '
...
<?php
	echo '
		<td colspan="2" class="rows span-5" style="font-weight: bold; text-align: right; padding: 13px 10px 13px 0;">Total General '.$valuta.'</td>
		<td class="rows" style="text-align: right; padding: 10px 5px 10px 0; font-weight: bold;">'.$factura['facturi']['total_general'].'</td>
	';
?>
		</tr>
<?php
